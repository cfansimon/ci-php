...
RUN curl -Lo /root/dotdeb.gpg https://www.dotdeb.org/dotdeb.gpg && apt-key add /root/dotdeb.gpg \
    && apt-get update \
    ...
    && curl -Lo /usr/local/bin/phpunit https://phar.phpunit.de/phpunit-5.7.phar  && chmod +x /usr/local/bin/phpunit \
